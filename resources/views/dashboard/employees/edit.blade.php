...
@endsection
@section('current-page', 'تعديل موظف')
@section('content')
        ...
        <div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
            <div class="card  box-shadow-0 ">
                <div class="card-header">
                    <h4 class="card-title mb-1">تعديل بيانات الموظف</h4>
                </div>
                <div class="card-body pt-0">
                    ...
                    <form action="{{route('dashboard.employees.update', 'test')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            <div class="form-group col-6">
                                ...
                                <input type="text" name="name" value="{{$employee->name}}" class="form-control" id="name" placeholder="أدخل الأسم">
                            </div>
                        ...
                        <div class="form-group col-6">
                            ...
                            <select name="job_grades_id" class="form-control select2-no-search" id="selectFormgrade"
                                ...
                                @foreach($jobgrades as $jobgrade)
                                    <option value="{{$jobgrade->id}}"{{$jobgrade->id == $employee->job_grades_id ? 'selected':"" }}>{{$jobgrade->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-group col-6">
                                ...
                                <input type="tel" class="form-control" value="{{$employee->phone}}" name="phone" id="exampleInputEmail1" placeholder="01111111">
                            </div>
                            <div class="form-group col-6">
                                <label for="exampleInputPassword1">هاتف أخر</label>
                                <input type="tel" class="form-control" value="{{$employee->alter_phone}}" name="alter_phone" id="exampleInputPassword1" placeholder="01111111">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-group col-6">
                                ...
                                <input class="form-control fc-datepicker" value="{{$employee->start_from}}" type="date" name="start_from" placeholder="YYYY-DD-MM">
                            </div>

                            <div class="form-group col-6">
                                <label for="exampleInputPassword1">تاريخ الميلاد</label>
                                <input class="form-control fc-datepicker" value="{{$employee->hiring_date}}" type="date" name="hiring_date" placeholder="YYYY-DD-MM" >
                            </div>
                            ...
                            <div class="form-group col-6">
                                ...
                                <select name="address_id" class="form-control select2">
                                    ...
                                    @foreach($addresses as $address)
                                        <option value="{{$address->id}}"{{$address->id == $employee->address_id ? 'selected':"" }}>{{$address->city}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-6">
                                ...
                                <option disabled selected="">افتح قائمة التحديد</option>
                                    @foreach($departments as $department)
                                        <option value="{{$department->id}}"{{$department->id == $employee->department_id ? 'selected':"" }}>{{$department->branch}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
